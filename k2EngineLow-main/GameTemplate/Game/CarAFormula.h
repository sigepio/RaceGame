...
public:
	...
	Vector4 CarSpeed(std::vector<std::vector<float>> data,				//トルクカーブのファイルパス
					const float SHIFT_UP_RPM_ADJUST,		//シフトアップ時の回転数調整量
					const float SHIFT_DOWN_RPM_ADJUST,		//シフトダウン時の回転調整量
					float currentRPM,						//今のエンジン回転数
					float velocity,						//今の時速
					float mass,							//車重
					float wheelRadius,						//タイヤの直径
					float grade,							//勾配(10%->0.1)
					float throttle_input,					//スロットル開度(1.0フルスロットル)
					std::vector<float> GEAR_RATIOS,		//ギア比
					float currentGear,						//今のギア
		            int MaxGear	,							//最大ギア数
					float AirPressure, 
